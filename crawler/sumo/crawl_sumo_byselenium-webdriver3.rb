...
class Crowler
  ...
  def self.parse(html,min_num,max_num)
    (min_num..max_num).map{|div_id|Nokogiri::HTML.parse(html).xpath("//*[@id='casset#{div_id}']/div/div[1]/div/h2/a").text}
  end
end
...
time = Benchmark.realtime do
  # @elements ||= []; Crowler::crawl(LIST_URL,1,1).each{|html|@elements = Crowler::parse(html,0,99)}
  # @elements ||= []; Crowler::crawl(LIST_URL,1,1).each{|html|@elements = Crowler::parse(html,0,99)}
  File.open('tags.txt','w+'){|result|result << Crowler::crawl(LIST_URL,1,1).map{|html|Crowler::parse(html,0,99)}}
  File.write('tags.txt',Crowler::crawl(LIST_URL,1,1).map{|html|Crowler::parse(html,0,99)})
  # File.open('tags.txt','w+'){|result|result << @elements}
end
